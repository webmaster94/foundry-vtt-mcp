...
export class FoundryDataAccess {
  ...
  async searchCompendium(
    ...
    for (const pack of packs) {
      try {
        // Foundry v13 returns the populated index Collection from getIndex().
        // Use that value directly instead of relying on pack.indexed/pack.index
        // mutation, while retaining the older API as a compatibility fallback.
        let packIndex: any;
        try {
  ...
  async switchScene(options: {
    ...
    const permissionCheck = permissionManager.checkWritePermission('document.execute');
    if (!permissionCheck.allowed) {
      throw new Error(`${ERROR_MESSAGES.ACCESS_DENIED}: ${permissionCheck.reason}`);
    }
    ...
    try {
      ...
      const previousScene = scenes.find((scene: any) => scene.active) || null;
      const result = {
        success: true,
      ...
      if (options.dryRun) {
        return {
          ...
          note: 'No scene was activated. Re-run without dryRun to commit.',
        };
      }
      ...
      // Persist the committed world mutation and inverse before attempting
      // optional browser-only viewport work. A failed canvas pan must never
      // make a successful activation look retryable or lose its undo record.
      await auditService.record({
        operation: 'scene.activate',
        toolName: 'switch-scene',
        documentRefs: [{ uuid: targetScene.uuid, id: targetScene.id, name: targetScene.name }],
        payloadSummary: {
          sceneIdentifier: options.scene_identifier,
          optimizeView: options.optimize_view !== false,
        },
        resultSummary: result,
        durationMs: Date.now() - start,
        success: true,
        ...(previousScene && previousScene.id !== targetScene.id
          ? {
              inverse: {
                kind: 'activate-scene' as const,
                ref: { uuid: previousScene.uuid },
              },
            }
          : {}),
      });
      ...
      let viewWarning: string | undefined;
      if (options.optimize_view !== false && typeof canvas !== 'undefined' && canvas?.scene) {
        ...
        if (width && height) {
          ...
          try {
            ...
            viewWarning = `Scene activated, but viewport centering failed: ${error instanceof Error ? error.message : String(error)}`;
            console.warn(`[${MODULE_ID}] ${viewWarning}`);
          }
        }
      }

      return viewWarning ? { ...result, viewWarning } : result;
    } catch (error) {
      await auditService.record({
        operation: 'scene.activate',
        toolName: 'switch-scene',
        payloadSummary: { sceneIdentifier: options.scene_identifier },
        durationMs: Date.now() - start,
        success: false,
