...
export class MacroManagementTools {
  ...
  getToolDefinitions(): Tool[] {
    return [
      this.tool('list-macros', 'List Foundry Macros.', {
        search: { type: 'string' },
        limit: { type: 'number', default: 50 },
      }),
      this.tool('get-macro', 'Get a Macro by uuid, id, or exact name.', {
        uuid: { type: 'string' },
        id: { type: 'string' },
        name: { type: 'string' },
      }),
      ...
      this.tool(
        'delete-macro',
        'Delete a Macro. Requires confirmDeletion=true.',
        {
          ref: { type: 'object' },
      ...
      this.tool(
        'execute-macro',
        'Execute a Macro in the connected GM browser. Resolve by uuid, id, or exact name.',
        {
          uuid: { type: 'string' },
          id: { type: 'string' },
          name: { type: 'string' },
    ...
    switch (name) {
      ...
      case 'get-macro':
        return this.query('getDocument', {
          ref: { ...this.parseMacroRef(args), documentType: 'Macro' },
        });
      case 'create-macro': {
      ...
      case 'update-macro': {
        ...
        return this.query('updateDocument', {
          ...parsed,
          ref: { ...parsed.ref, documentType: 'Macro' },
        });
      }
      case 'delete-macro': {
